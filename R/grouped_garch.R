...
function(object, ...)
{
    if(!inherits(object, "grouped_garch"))
...
function(object, ...)
{
    ...
    class(ans) <- "summary.grouped_garch"
    return(ans)
}

plot.grouped_garch <- function(x, ask = interactive(), ...)
{
    if(!inherits(x, "grouped_garch"))
        stop("method is only for garch objects")
    op <- par()
...
print.summary.grouped_garch <-
function(x, digits = max(3, getOption("digits") - 3),
         signif.stars = getOption("show.signif.stars"), ...)
{
    if(!inherits(x, "summary.grouped_garch"))
        stop("method is only for summary.grouped_garch objects")
    cat("\nCall:\n", deparse(x$call), "\n", sep = "")
